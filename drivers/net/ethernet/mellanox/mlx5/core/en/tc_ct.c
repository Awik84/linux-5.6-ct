...
#define MLX5_TC_CT_TABLE_SIZE (1024 * 1024 * 4)
			  ...
			  bool nat)
{
	...
	kfree(spec);
	ct_dbg("Offloaded ct entry rule in zone %d with counter id %d", entry->zone, mlx5_fc_id(entry->counter));
...
mlx5_tc_ct_create_ct_table(struct mlx5_eswitch *esw, bool ipv4, bool tcp)
{
	...
	struct mlx5_tc_ct_table *ct_ft;
	struct mlx5_flow_handle *rule;
	struct mlx5_flow_table *fdb;
	void *headers_c, *fg_in;
	int err;

	ct_ft = kzalloc(sizeof(*ct_ft), GFP_KERNEL);
	fg_in = kvzalloc(MLX5_ST_SZ_BYTES(create_flow_group_in), GFP_KERNEL);
	if (!ct_ft || !fg_in) {
		err = -ENOMEM;
		goto err_alloc;
	}
	...
	ft_attr.max_fte = MLX5_TC_CT_TABLE_SIZE;
	...
	headers_c = MLX5_ADDR_OF(create_flow_group_in, fg_in,
	...
	if (tcp) {
		...
		MLX5_SET_TO_ONES(fte_match_set_lyr_2_4, headers_c, tcp_flags);
	} else {
	...
	memset(headers_c + mlx5e_tc_attr_to_reg_mappings[ZONE_TO_REG].soffset,
	       0xFF, mlx5e_tc_attr_to_reg_mappings[ZONE_TO_REG].mlen);
	MLX5_SET(create_flow_group_in, fg_in, match_criteria_enable,
		 MLX5_MATCH_OUTER_HEADERS | MLX5_MATCH_MISC_PARAMETERS_2);
	MLX5_SET(create_flow_group_in, fg_in, end_flow_index,
		 fdb->max_fte - 2);
	main_group = mlx5_create_flow_group(fdb, fg_in);
	if (IS_ERR(main_group)) {
		err = PTR_ERR(main_group);
		goto err_main_group;
	}

	memset(fg_in, 0, MLX5_ST_SZ_BYTES(create_flow_group_in));
	MLX5_SET(create_flow_group_in, fg_in, start_flow_index,
		 fdb->max_fte - 1);
	MLX5_SET(create_flow_group_in, fg_in, end_flow_index,
		 fdb->max_fte - 1);

	miss_group = mlx5_create_flow_group(fdb, fg_in);
	...
	dest.ft = mlx5_esw_chains_get_tc_end_ft(esw);
	rule = mlx5_add_flow_rules(fdb, NULL, &flow_act, &dest, 1);
	if (IS_ERR(rule)) {
		err = PTR_ERR(rule);
		goto err_miss_rule;
	}
	...
	ct_ft->fdb = fdb;
	ct_ft->main_group = main_group;
	ct_ft->miss_group = miss_group;
	ct_ft->miss_rule = rule;
...
err_miss_group:
	mlx5_destroy_flow_group(main_group);
err_main_group:
...
static void
mlx5_tc_ct_destroy_ct_table(struct mlx5_tc_ct_table *ct_ft)
{
	mlx5_del_flow_rules(ct_ft->miss_rule);
	mlx5_destroy_flow_group(ct_ft->miss_group);
